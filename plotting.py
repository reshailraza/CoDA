...
def plot_result_ablation2(s_num, dataset_name='training', color_range='normal', vmin=None, vmax=None,
    ...
    # ── Compute color limits from ground truth only ──────────────────────────
    ...
    # ── Apply ground-truth color limits to ALL panels in each row ────────────
    for im in images[:4]:
        im.set_clim(vmin=gt_vmin[0], vmax=gt_vmax[0])
    for im in images[4:]:
        im.set_clim(vmin=gt_vmin[1], vmax=gt_vmax[1])

    # ── One colorbar per predicted panel (cols 1-3), locked to GT range ──────
    # Top row: CAFNet=axes[1], Fine-Tuned=axes[2], CoDA=axes[3]
    for im, ax in zip(images[0:4], axes[0:4]):
        fig.colorbar(im, ax=ax, orientation='horizontal', fraction=.1)

# Bottom row: all 4 panels including Ground Truth
